...
@AllArgsConstructor
public class JobApplicationInstituteSpecificaiton implements Specification<JobApplication> {
    ...
    public Predicate toPredicate(Root<JobApplication> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        if (institutes == null)
            ...
            Join<Qualification, Degree> degree = qualification.join("institute");
            return degree.get("id").in(institutes);
        }
    }
